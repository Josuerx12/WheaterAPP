...
searchLocation.addEventListener("click", () => {
    ...
    inputSearch.value = "";
   handleSearchLocation(inputLocationValue)
});
...
function handleSearchLocation(location) {
    ...
    .then((weather) => {
        ...
        updateTemperatureUnits(currentTemp)
        updateSidebarInformation(weatherData)

        const highlights = weatherData.forecast.forecastday;
        updateHighlightsSection(highlights);
      
        updateWindSection(weatherData.current.wind_degree, weatherData.current.wind_mph)
        
      //humidity
        updateHumiditySection(weatherData.current.humidity);
        //visibility
...
function updateTemperatureUnits(currentTemperatureInCelsius) {
    const currentTempF = Number((currentTemperatureInCelsius * 1.8) + 32).toFixed(0);
    ...
    celciusBtn.addEventListener("click", () => {
      temperature.innerText = `${currentTemperatureInCelsius}`;
      unit3.innerText = "°C"
...
function updateSidebarInformation(weatherData) {
...
function updateHighlightsSection(highlights) {
    highlights.foreach((tempo) => {
...
function updateWindSection(windDegree, windMph) {
    const windForce = document.querySelector(".wind-force");
    const windDirection = document.querySelector("#wind-direction");
    const windArrowDirection = Number(315 + windDegree)
    windForce.innerHTML = `<p class="wind-force">${windMph} <span>mph</span></p>`;
...
function updateHumiditySection(humidity) {
    const percentageHum = document.querySelector("#percentage");
    const humidityPer = document.querySelector(".humidity-percentage");
    humidityPer.innerHTML = `<p class="humidity-percentage">${humidity}<span>%</span></p>`;
    percentageHum.style.width = `${humidityy}%`;
}
